...
Another form of bias that can be introduced into a dataset is called lecft-truncation. (Also a form of censorship). 
This occurs when individuals may die even before ever entrying into the study. Both  ``KaplanMeierFitter`` and ``NelsonAalenFitter`` have an optional argument for ``entry``, which is an array of equal size to the duration array.
